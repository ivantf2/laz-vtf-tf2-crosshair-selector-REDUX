...
def get_crosshairs():
    ...
    if not os.path.isdir(xhairs_path):
        return []
    
    #TODO: add support for default crosshairs
    xhair_vtfs = ["crosshairs"]
    for file_name in os.listdir(xhairs_path):
        if file_name.endswith(".vtf"):
            xhair_vtfs.append(file_name[:-4])
    #files = [f for f in os.listdir(xhairs_path) if os.path.isfile(os.path.join(xhairs_path, f))]
...
def get_xhair_display_path(xhair):
    ...
    if os.path.isfile(path2): # for .exe file
        return path2

    if os.path.isfile(path1): # for .py script
        return path1
